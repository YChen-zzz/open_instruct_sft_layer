...
os.environ["TORCH_LOADS_WEIGHTS_ONLY"] = "0"
@dataclass
class FlatArguments:
    """
    ...
    save_state_once_at_step: Optional[int] = field(
        default=None,
        metadata={
            "help": "If set, save full accelerator state once when global completed_steps reaches this value, then exit."
        },
    )
    save_state_then_exit: bool = field(
        default=False,
        metadata={
            "help": "If True and save_state_once_at_step is reached, exit training immediately after saving state."
        },
    )
    ...
    reset_lr_scheduler_after_resume: bool = field(
        default=False,
        metadata={
            "help": "If True, after accelerator.load_state, rebuild LR scheduler from step 0."
        },
    )
        
    reset_training_process: bool = field(
        default=False,
        metadata={
            "help": "If True, after accelerator.load_state, set current step to be 0."
        },
    )
    new_learning_rate_after_resume: Optional[float] = field(
        default=None,
        metadata={
            "help": "If set, override optimizer param_group['lr'] to this value after resume (before creating scheduler if resetting)."
        },
    )
    optimizer_set_alpha_after_resume: Optional[float] = field(
        default=None,
        metadata={
            "help": "If set and optimizer supports 'alpha' in param groups, override it after resume."
        },
    )
    
    use_AdamW_MoFO: bool = field(
        default=False, metadata={"help": "use MoFO"}
    )
    
    MoFO_fraction: float = field(
        default=0.2, metadata={"help": 'select how many parameters to update in each iteration (eg. 0.03 means 3% parameters update)'}
    )

    trainable_layers: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Comma-separated list of decoder layer indices (0-indexed, matching "
                "model.model.layers[i]) to unfreeze for training; all other parameters "
                "(other layers, embeddings, lm_head, norm) are frozen. "
                "If None (default), all parameters are trainable as before. "
                "Example: '0,1,2'"
            )
        },
    )
...
def main(args: FlatArguments, tc: TokenizerConfig):
    ...
    if args.trainable_layers is not None:
        layer_ids = {int(x) for x in args.trainable_layers.split(",") if x.strip() != ""}
        logger.info(f"Freezing all parameters except decoder layers {sorted(layer_ids)}")
        for name, param in model.named_parameters():
            m = re.search(r"\.layers\.(\d+)\.", name)
            param.requires_grad = bool(m) and int(m.group(1)) in layer_ids
        n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        n_total = sum(p.numel() for p in model.parameters())
        logger.info(f"Trainable params: {n_trainable} / {n_total} ({100 * n_trainable / n_total:.2f}%)")
    ...
    #if not args.reset_lr_scheduler_after_resume:
    ...
    if last_checkpoint_path:
        ...
        if "epoch" in training_difference:
            ...
            resume_batch_idx = int(training_difference.replace("step_", "").replace("_state", "")) * args.gradient_accumulation_steps
        ...
        # Optional overrides immediately after resume
        if args.new_learning_rate_after_resume is not None:
            for param_group in optimizer.param_groups:
                param_group["lr"] = float(args.new_learning_rate_after_resume)
            accelerator.print(f"Overrode optimizer learning rate to {args.new_learning_rate_after_resume} after resume.")

        if args.optimizer_set_alpha_after_resume is not None:
            for param_group in optimizer.param_groups:
                if "fraction" in param_group:
                    param_group["fraction"] = float(args.optimizer_set_alpha_after_resume)
            accelerator.print("Applied optimizer 'alpha' override after resume if supported by optimizer.")

        if args.reset_lr_scheduler_after_resume:
            if accelerator._schedulers:
                for sche in accelerator._schedulers:
                    sche.last_epoch = -1
            #    del accelerator._schedulers  # Assuming you have only one scheduler; adjust index if multiple
            #
            #accelerator._schedulers = []
            #
            #
            #model.lr_scheduler.last_epoch = -1  # Use -1 to fully reset to initial state (for linear scheduler, starts warmup from scratch)
            lr_scheduler.last_epoch = -1
            #accelerator.print("Reset LR scheduler to step 0 after resume as requested.")
            # Optionally reset optimizer LR if needed (you already have this commented)
            # for param_group in optimizer.param_groups:
            #     param_group["lr"] = float(args.learning_rate)
        #
            ## Rebuild the scheduler from scratch
            ##lr_scheduler = get_scheduler(
            ##    name=args.lr_scheduler_type,
            ##    optimizer=optimizer,
            ##    num_training_steps=num_training_steps_for_scheduler,
            ##    num_warmup_steps=num_warmup_steps,
           #)##
            ##print(f"num_training_steps_for_scheduler: {num_training_steps_for_scheduler} num warm up: {num_warmup_steps}")
        ##
        #   ### Prepare the new scheduler for distributed compatibility
            ##lr_scheduler = accelerator.prepare(lr_scheduler)
        ##
        #   ### Add the new prepared scheduler to Accelerate's internal list
            ##accelerator._schedulers.append(lr_scheduler)
        ##
        #   ### Register it for future checkpointing
            ##accelerator.register_for_checkpointing(lr_scheduler)
        ##
        #   ### Reset its state to start from step 0
            ##lr_scheduler.last_epoch = 0
        ##
            ##accelerator.print("Rebuilt and replaced LR scheduler from step 0 after resume as requested.")
    ...
    # Whether we've performed the one-time state save
    did_save_state_once = False
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                # One-time accelerator state save and optional early exit
                if (
                    not did_save_state_once
                    and args.save_state_once_at_step is not None
                    and completed_steps == int(args.save_state_once_at_step)
                ):
                    state_dir_name = f"step_{completed_steps}_state"
                    output_dir = state_dir_name if args.output_dir is None else os.path.join(args.output_dir, state_dir_name)
                    accelerator.print(
                        f"save_state_once_at_step reached at step {completed_steps}. Saving full accelerator state to {output_dir}"
                    )
                    # Ensure directory exists
                    if output_dir is not None:
                        os.makedirs(output_dir, exist_ok=True)
                    # Save full training state (model, optimizer, lr scheduler, rng, etc.)
                    accelerator.save_state(output_dir)
                    accelerator.wait_for_everyone()
                    did_save_state_once = True
                    # Optionally exit immediately
                    if args.save_state_then_exit:
                        accelerator.print("Exiting after one-time state save as requested.")
                        # Graceful termination: break out of loops
                        break
                ...
                if completed_steps >= args.max_train_steps or (did_save_state_once and args.save_state_then_exit):
                    break

        if args.save_state_then_exit and did_save_state_once:
            # Exit outer epoch loop immediately after saving state
            break
    ...
    # If we exits early due to one-time state save, skip final weight save and cleanup
    if args.save_state_then_exit and did_save_state_once:
        return
