...
def get_tokenizer_tulu_v2_1(tc: "TokenizerConfig"):
    ...
    """
    Yupeng: 
    不用他自带的chat template 关键点在这里：
    只要chat_template_name not in CHAT_TEMPLATES 他就会使用tokenizer 自带的chat template
...
def get_tokenizer_tulu_v2_2(tc: "TokenizerConfig"):
    ...
    if "olmo" in config.model_type:
        ...
        elif "olmoe" not in config.model_type:
    ...
    if tc.chat_template_name in CHAT_TEMPLATES:
        ...
        try:
            ...
            print("use default chat template")
        except Exception:
            raise ValueError(f"Could not find chat template for {tc.tokenizer_name_or_path}.")
...
def sft_tulu_tokenize_and_truncate_v1(row: Dict[str, Any], tokenizer: PreTrainedTokenizer, max_seq_length: int):
    ...
    """
    Yupeng: 8.31
    这个encoding 方式 默认用了类似于
    
    [
    {
    "role": "user",
    "content": "Answer this question: where did the third punic war take place??\n\nTunisia\n\n\nAnswer this question: who played the leprechaun in leprechaun in the hood??\n\nWarwick Davis\n\n\nAnswer this question: secret train tunnel from grand central to the waldorf-astoria??\n\nTrack 61\n\n\nAnswer this question: where do tornadoes happen most in the world??\n\n"
    },
    {
    "role": "assistant",
...
def get_cached_dataset_tulu_with_statistics(
    ...
    if dataset_config_hash is None:
        ...
        for i in range(0, len(dataset_mixer_list), 2):
            ...
            """
            Yupeng: 8.31
            关键是这个dataset_transform_fn， 他会选择不同的decoding函数
