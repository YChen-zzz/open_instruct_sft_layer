...
This repository is derived from [allenai/open-instruct](https://github.com/allenai/open-instruct), licensed under the [Apache License 2.0](./LICENSE). See that repository and the papers linked below for the original project and citation information.

Modifications in this fork include, notably, `open_instruct/finetune.py` (layer-wise trainable-parameter freezing, see [Layer-wise Training](#layer-wise-training) below) and the homework scripts under `scripts/`.
...
`open_instruct/finetune.py` supports restricting training to a specific subset of decoder layers via `--trainable_layers`, a comma-separated list of 0-indexed layer indices (matching `model.model.layers[i]`). When set, every parameter outside those layers (other decoder layers, embeddings, `lm_head`, final norm) is frozen (`requires_grad=False`); when omitted, all parameters remain trainable as before.

`scripts/finetune_with_accelerate_homework_qwen4b_layers.sh` uses this to sweep several layer-wise finetuning configurations on `Qwen3-4B-Base`, running each case back-to-back and evaluating with ifeval after each run:

```bash
bash scripts/finetune_with_accelerate_homework_qwen4b_layers.sh
```

It trains one full-parameter `baseline` case plus three layer-frozen cases (`layers_0_1_2`, `layers_16_17_18`, `layers_33_34_35` — early/middle/late decoder layers only), which is useful for comparing how finetuning a small band of layers affects downstream instruction-following behavior versus full-parameter SFT.
