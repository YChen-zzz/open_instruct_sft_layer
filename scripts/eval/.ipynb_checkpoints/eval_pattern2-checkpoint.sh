...
###############################################################################################################################SFT Eval #######################################################
model_path="/chenyupeng/model_files/openinstruct/llama32_3B_2e_2e-5_mofo015_resumeAdam_from360/finetune"
...
for step in $(seq 480 120 1200); do
    filename="step_${step}"
...
for item in "${model_list[@]}"; do
    ...
    #torchrun --nproc-per-node=2 --master_port=25903 --no-python lm_eval --model hf \
