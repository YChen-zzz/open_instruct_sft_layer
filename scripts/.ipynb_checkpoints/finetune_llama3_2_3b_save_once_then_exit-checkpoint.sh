...
base_output_path=/chenyupeng/model_files/openinstruct/llama32_3B_save_once_exit
...
# Set the global step at which to save full accelerator state once and then exit
#SAVE_STATE_AT_STEP=60  # change as needed
#
#accelerate launch \
...
#    --save_state_once_at_step $SAVE_STATE_AT_STEP \
#    --save_state_then_exit True &> >(tee -a $out_path/log.log)
...
accelerate launch \
    ...
    --reset_training_process True \
    --reset_lr_scheduler_after_resume True \
    --resume_from_checkpoint "$out_path/finetune/step_60" \
    --keep_last_n_checkpoints -1 \
